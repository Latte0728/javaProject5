...
//	Vector<String>	vData	=	null;
	...
	public T1() {
		...
		JButton btnNewWindow = new JButton("새 창에 선택 값 띄우기");
		
		btnNewWindow.setFont(new Font("굴림", Font.PLAIN, 18));
		btnNewWindow.setBounds(24, 10, 268, 50);
...
//		scrollPane.setViewportView(tbl);
	...
	// 4. DefaultTableModel에 담긴 Vector 형식의 자료와 타이틀로 JTable 생성시켜준다.(생성시 담아준다.)
	tbl	=	new	JTable(dtm);
	...
	btnNewWindow.addActionListener(new ActionListener() {
		...
		vo.setIdx((int)tbl.getValueAt(row, 0)); // int row	=	tbl.getSelectedRow();에서 가져옴
		vo.setName(tbl.getValueAt(row, 1).toString()); // int row	=	tbl.getSelectedRow();에서 가져옴
		vo.setIdx((int)tbl.getValueAt(row, 2));
